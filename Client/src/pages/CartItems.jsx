...
const CardItems = () => {
    ...
    const{ products ,currency , CardItems, updateCart,  getCartAmount, removeFromCart,getCartItemsCount,navigate} = useAppContext();
        ...
        <div className="flex flex-col md:flex-row py-16 max-w-6xl w-full px-6 mx-auto">
            <div className='flex-1 max-w-4xl'>
                <h1 className="text-3xl font-medium mb-6">
                    Shopping Cart <span className="text-sm text-indigo-500">{getCartItemsCount()}</span>
                </h1>
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        ...
                        <button onClick={() => removeFromCart(product._id)}  className="cursor-pointer mx-auto">
                           <img className="w-6 h-6" src={assets.remove_icon} alt="delete icon" />
                        </button>
                    </div>)
                )}
